...
void calc_theo( double x, double Q2, double &theo_he3, double &theo_h3 , const double *pars , TH1D* hist );
void calc_theo_rho( double x, double Q2, double &theo_he3, double &theo_h3 , const double *pars , TH1D* hist );
...
int main(int argc, char ** argv){
	...
	// TH1 for the F2bound object
	TH1D * f2bound_sf = new TH1D("f2bound_sf","f2bound_sf",100,0,3);
	TH1D * f2bound_sf_noOff = new TH1D("f2bound_sf_noOff","f2bound_sf_noOff",100,0,3);
	...
	calc_theo_rho(x,		Q2,	he3,	h3,	pars_cstOff_rho,		f2bound_sf);
	// Resulting constant offshell fit from SF and rho with offshell FORCED to 0
	//calc_theo(x,		Q2,	he3,	h3,	pars_cstOff_no_SF,	f2bound_sf_noOff);
	
	outFile->cd();
	f2bound_sf->Write();
	//f2bound_sf_noOff->Write();
	outFile->Close();	
	...
	for(	itK_n = test_n.begin(), itK_p = test_p.begin(); 
		...
		for( itE_n = itK_n->second.begin(), itE_p = itK_p->second.begin(); 
			...
			for( double theta = 0 ; theta <= M_PI ; theta += dTheta){
				...
				Z = 2; N = A-Z;
				theo_pro += jacobian * flux_fact * phi_int * mass_fact * dTheta
		...
		cout << theo_pro << " " << theo_neu << "\n";
		
		hist->Fill( p_m , theo_pro );
...
void calc_theo_rho( double x, double Q2, double &theo_he3, double &theo_h3 , const double *pars , TH1D* hist ){
	...
	for( int i = 0 ; i < rho_p.size() ; i++ ){
			...
			theo_he3 += ( Z*sp_p*offshell(nu,x/alpha,pars[3]) 
				...
				* F2p->Eval(x/alpha, Q2 )
				* pars[4];
			double theo_pro = ( Z*sp_p*offshell(nu,x/alpha,pars[3]) ) * F2p->Eval(x/alpha, Q2 ) * (1./F2p->Eval(x, Q2)) * (1./Z);
