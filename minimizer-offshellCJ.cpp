...
double offshell( double virt, double xB , double off_a , double off_b, double off_c );
...
int main(int argc, char ** argv){
	...
	const double of_a 	= -0.25;
	const double of_b 	= 0.05;
	const double of_c 	= 0.4;
	const double Nhe3 	= 1.;
	const double Nh3 	= 1.;
	...
	min->SetVariable(5,	"of_c",		of_c, 		0.1	);
	min->SetVariable(6,	"N_he3",	Nhe3,   	0.1	);
	min->SetVariable(7,	"N_h3",		Nh3,    	0.1	);
	opt = atoi(argv[1]);
	if( opt == 1 ) min->FixVariable(6);
	if( opt == 2 ) min->FixVariable(7);
...
double offshell( double virt, double xB , double off_a , double off_b, double off_c ){
	return 1 + virt*virt*off_a*(xB-off_b)*(xB-off_c)*(1.+off_b-xB);
}
double f2nf2p( double xB , double f2nf2p_a, double f2nf2p_b, double f2nf2p_c ){
...
double Chi2( const double *pars ){
	...
	// par 3-5 	= virt parameter
	// par 6-7	= norm uncertainty
	...
	for( int i = 0 ; i < 22 ; i++ ){
			...
			for( itE_n = itK_n->second.begin(), itE_p = itK_p->second.begin(); 
				...
				for( double theta = 0 ; theta <= M_PI ; theta += dTheta){
					...
					theo_he3 += jacobian * flux_fact * phi_int * mass_fact * dTheta 
						* ( Z*sp_p*offshell(nu,x/y,pars[3],pars[4],pars[5]) 
							+ N*sp_n*offshell(nu,x/y,pars[3],pars[4],pars[5])*f2nf2p(x/y,pars[0],pars[1],pars[2]) )
						* F2p->Eval(x/y, Q2 );
					Z = 1; N = A-Z; // H3 - as it's He3 SF, swap p and n for only the SF
					theo_h3 += jacobian * flux_fact * phi_int * mass_fact * dTheta 
						* ( Z*sp_n*offshell(nu,x/y,pars[3],pars[4],pars[5]) 
							+ N*sp_p*offshell(nu,x/y,pars[3],pars[4],pars[5])*f2nf2p(x/y,pars[0],pars[1],pars[2]) )
						* F2p->Eval(x/y, Q2 );
		...
		// Calculate chi2:
		if( opt == 0 || opt == 2 ){
			chi2 += 	pow(	(data_he3[i] - pars[6]*theo_he3) /data_he3_er[i]	, 2 );
			chi2 +=		pow(	(pars[6] - 1.)/0.05	, 2 );	// He-3 normalization 
		}
		if( opt == 0 || opt == 1 ){
			chi2 += 	pow(	(data_h3[i]  - pars[7]*theo_h3)  /data_h3_er[i]	, 2 );
			chi2 += 	pow(	(pars[7] - 1.)/0.05	, 2 );  // H-3 normalization
		}
